 ...
 * @param deps - credential source overrides, for tests that must not touch the
 * keychain or the network. Production always uses the real ones.
 * @returns a machine-readable outcome for the pass.
 */
export async function syncCredential(ctx, options, state, deps = {}) {
	const read = deps.read ?? readClaudeCredential;
	const renew = deps.renew ?? refreshClaudeCredential;
	const configDir = resolveClaudeConfigDir(undefined);
	const credential = await read(options, configDir);
